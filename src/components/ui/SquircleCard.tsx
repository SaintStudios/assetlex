 ...
 * Parameterized by --card-clip-radius / --card-clip-handle; steps up at sm:.
 */
...
export interface SquircleCardProps {
  ...
  seeAllHref?: string;
  onSeeAll?: () => void;
  /** Small chip rendered at the right end of the title strip. */
...
export function SquircleCard({
  ...
  const hasStrip = title !== undefined || chip !== undefined || seeAllHref !== undefined || onSeeAll !== undefined;
  return (
    <SquircleSurface
      className={cn(
    ...
    >
      {hasStrip && (
        <div className="flex items-center justify-between gap-2 pb-2 pl-3.5 pr-2 pt-1.5">
          ...
          {onSeeAll ? (
            <button type="button" onClick={onSeeAll} className={cn(seeAllClass, "cursor-pointer")}>
              See all {arrow}
            </button>
          ) : seeAllHref ? (
            <a href={seeAllHref} className={seeAllClass}>
              See all {arrow}
            </a>
          ) : null}
          {chip}
      ...
      <SquircleSurface
        className={cn(
          "overflow-hidden rounded-[22px] border border-border bg-[#f6f6f6] shadow-[0_1px_2px_rgba(0,0,0,0.06)] [--card-clip-radius:12px] sm:rounded-[44px] sm:[--card-clip-radius:17px]",
          "dark:bg-background",
          contentClassName,
        )}
      >
